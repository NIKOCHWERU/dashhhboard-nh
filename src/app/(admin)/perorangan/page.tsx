...
export default function PeroranganPage() {
  ...
  const isAdminOrPIC = user?.role === "admin" || user?.canManagePerorangan;
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/progress-pekerjaan?type=NON_RETAINER_${activeTab}`);
      if (res.ok) {
        const list = await res.json();
        setData(list);
      }
  ...
  const handleExportExcel = () => {
    try {
      ...
      xlsx.utils.book_append_sheet(workbook, worksheet, `NON_RETAINER_${activeTab}`.substring(0, 30));
      xlsx.writeFile(workbook, `Non_Retainer_${activeTab}_${new Date().toISOString().split("T")[0]}.xlsx`);
    } catch (e) {
      ...
      reader.onload = async (evt) => {
        try {
          ...
          for (let i = 1; i < rows.length; i++) {
            ...
            // Always use RETAINER column logic for NON_RETAINER_PK/PU
  ...
  const getFormFields = () => {
    const list: { key: string; label: string; type: string; options?: string[] }[] = [
      ...
      { key: "waktu", label: "Waktu (Jam)", type: "text" },
      { key: "namaKlien", label: "Nama Perusahaan / Klien", type: "text" },
      { key: "quadran", label: "Quadran (Q1 / Q2 / Q3)", type: "select", options: ["Q1", "Q2", "Q3"] },
      { key: "deskripsi", label: "Deskripsi Pekerjaan", type: "textarea" },
      { key: "tugas", label: "Tugas / Instruksi", type: "textarea" },
      { key: "area", label: "Area / Wilayah", type: "text" },
      { key: "status", label: "Status Kerja", type: "select", options: STATUS_METADATA.map(s => s.key) },
      { key: "keterangan", label: "Keterangan (Fakta Objektif)", type: "textarea" },
      { key: "catatan", label: "Catatan (Tambahan / Review)", type: "textarea" }
    ];
    return list;
  };
    ...
    <div className="space-y-6 print:space-y-0 print:p-0">
      ...
      <div className="hidden print:block text-center mb-6 text-black">
        <h2 className="text-sm font-black uppercase tracking-wider">
          Manajemen Klien Non Retainer ({activeTab})
        </h2>
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-800 gap-1 overflow-x-auto pb-px print:hidden mt-2">
        {(["PK", "PU"] as ActiveTab[]).map((tab) => {
          const isActive = activeTab === tab;
          let label = tab === "PK" ? "PK ( Pekerjaan Khusus )" : "PU ( Pekerjaan Umum )";
          let icon = null;
