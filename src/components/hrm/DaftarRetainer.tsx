    ...
    <div className="space-y-4">
      ...
      <div className="flex items-end justify-between pb-2 border-b border-stroke dark:border-strokedark">
        ...
        <div className="flex gap-2">
          <button onClick={handleImport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-none border border-stroke bg-white text-xs font-bold text-gray-700 hover:bg-gray-50 dark:border-strokedark dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 transition-all shadow-sm uppercase tracking-wider">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-none w-full shadow-2xl border border-stroke dark:border-strokedark">
          ...
          <div className="p-5 grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input type="text" value={formData.clientName} onChange={e => setFormData({...formData, clientName: e.target.value})} className="w-full p-2.5 border border-stroke rounded-none dark:bg-gray-900 dark:border-strokedark dark:text-white text-xs outline-none focus:border-brand-500 font-medium" />
            </div>
            <div className="col-span-2">
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Nama Project</label>
              <input type="text" value={formData.projectName} onChange={e => setFormData({...formData, projectName: e.target.value})} className="w-full p-2.5 border border-stroke rounded-none dark:bg-gray-900 dark:border-strokedark dark:text-white text-xs outline-none focus:border-brand-500 font-medium" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Tanggal Mulai</label>
              <input type="date" value={formData.startDate} onChange={e => setFormData({...formData, startDate: e.target.value})} className="w-full p-2.5 border border-stroke rounded-none dark:bg-gray-900 dark:border-strokedark dark:text-white text-xs outline-none focus:border-brand-500 font-medium" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1">Tanggal Berakhir</label>
              <input type="date" value={formData.endDate} onChange={e => setFormData({...formData, endDate: e.target.value})} className="w-full p-2.5 border border-stroke rounded-none dark:bg-gray-900 dark:border-strokedark dark:text-white text-xs outline-none focus:border-brand-500 font-medium" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" value={formData.contractValue} onChange={e => setFormData({...formData, contractValue: e.target.value})} className="w-full p-2.5 border border-stroke rounded-none dark:bg-gray-900 dark:border-strokedark dark:text-white text-xs outline-none focus:border-brand-500 font-medium" />
            </div>
          </div>
          <div className="p-5 border-t border-stroke dark:border-strokedark flex justify-end gap-3 bg-gray-50 dark:bg-gray-900/50">
            ...
            <button onClick={handleSave} className="px-4 py-2 bg-brand-500 text-white text-xs font-bold rounded-none hover:bg-brand-600 transition-colors shadow-sm">Simpan</button>
          </div>
        </div>
      </Modal>

      {/* DETAIL MODAL (GLOBAL REQUEST ACTION VIEW) */}
      <Modal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} className="max-w-xl !rounded-none !bg-transparent !p-0">
        {selectedItem && (
